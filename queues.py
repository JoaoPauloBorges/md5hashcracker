...
def worker(running_flag, inq, outq):
    while True:
        if running_flag.value != 0:
            # if not inq.empty() or inq.qsize() > 0:
            data = inq.get()
            fn, arg = data
            output = fn(running_flag, arg[0], arg[1])
            if output != None:
                outq.put(output)
        else:
            exit()
...
if __name__ == '__main__':
    ...
    teste = hashlib.md5('!!!!'.encode('utf-8')).hexdigest()
    blocks = []  
    nLines = 0
    maxLines = 5000
    temp = ""
    ...
    if not tasks._closed:
        if tasks.qsize() > 0 or not tasks.empty():
            while tasks.qsize() > 0:
                time.sleep(2)
        tasks.close()
        tasks.join_thread()
